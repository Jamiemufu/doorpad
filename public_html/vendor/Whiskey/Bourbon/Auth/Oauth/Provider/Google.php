...
class Google extends OauthAbstract
{
    ...
    /**
     * Instantiate a Google OAuth object
     ...
     * @return self       Google object for chaining
    ...
    /**
     * Get Google authorisation URL for user to click through to
     * @param  array  $scope Array of permissions to request
     * @return string        Google authorisation URL
    ...
    public function getLoginUrl(array $scope = [])
    {
        ...
        $base_scope = ['openid', 'email'];
        $scope      = array_merge($scope, $base_scope);
        $scope      = array_unique($scope);
        $scope      = implode(' ', $scope);
        $scope      = urlencode($scope);

        return 'https://accounts.google.com/o/oauth2/auth?client_id=' . urlencode($this->_client_id) . '&amp;response_type=code&amp;scope=' . $scope . '&amp;redirect_uri=' . urlencode($this->_redirect_uri) . '&amp;state=' . urlencode($this->_dependencies->csrf->generateToken());

    }


    /**
     * Validate a Google OAuth callback and return user information
    ...
    public function fetch()
    {
        ...
        if (isset($_GET['error']))
        {
            throw new AuthorisationFailedException('Google authorisation failed');
        }
            ...
            isset($_GET['state']))
        {
            ...
            if ($this->_dependencies->csrf->checkToken($_GET['state']))
            {
                 ...
                 * Get an access token
                 */
                $token_data = $this->_dependencies->http->post('https://accounts.google.com/o/oauth2/token', $params);
                    ...
                    isset($token_data->access_token))
                {
                     ...
                     * Get user details
                     */
                    $user_info = $this->_dependencies->http->get('https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=' . urlencode($token_data->access_token));
